...
use aes::cipher::{KeyInit, BlockCipher,BlockEncrypt,BlockDecrypt};
...
impl FixedKeyPrgStream {
    ...
    pub fn next_bits(&mut self,len: usize) -> Vec<bool> {
        ...
        let block_num = len/64;

        for i in 0..block_num+1{

            out.append(&mut u64_to_bits(self.next_u64()));
    ...
    fn load(key: &[u8; 16] ) -> __m128i {
        ...
        // let array:GenericArray<u8,16usize>  = GenericArray::from_slice(key);
        // let array:GenericArray<u8,16>  = GenericArray::from_slice(key);
        // let array  = GenericArray<[u8;16]>::from_slice(key);
        // let array = key.into();
        ...
        unsafe {
            ...
            // _mm_loadu_si128(array.as_ptr() as *const __m128i)
        }
    }
}
