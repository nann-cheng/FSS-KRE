...
mod tests {
    ...
    /*async fn max_works_together()->(Vec<bool>, Vec<bool>)
    {
        let seed = PrgSeed::random();
        let mut stream = FixedKeyPrgStream::new();
        stream.set_key(&seed.key);
        let x0 = stream.next_bits(10*32);
        let x1 = stream.next_bits(10*32);

        let config0 = FileConfig{
            dir_path: "../data",
            a_file: "a0.bin",
            k_file: "k0.bin",
            qa_file: "qa0.bin",
        ...
        let offlinedata1 = OfflineInfomation::new();
        let offlinedata2 = OfflineInfomation::new();
        
        let mut p0 = MPCParty::new(offlinedata1, PartyRole::Active);
        let mut p1 = MPCParty::new(offlinedata2, PartyRole::Passitive);
        p0.setup(&config0, 10, 32);
        p1.setup(&config1, 10, 32);

        let (msg_tx, mut msg_rx) = mpsc::channel::<Vec<u8>>(1024);
        let (msg_ty, mut msg_ry) = mpsc::channel::<Vec<u8>>(1024);
        let f1 = max(&p0, &x0,  &msg_ty, &mut msg_rx);
        let f2 = max(&p1, &x1,  &msg_tx, &mut msg_ry);
        let(c1, c2) = join!(f1, f2); 
        (c1, c2)
    }

    #[tokio::test]
    async fn max_works(){
        //let (c1, c2) = block_on(max_works_together());
        let (c1, c2) = max_works_together().await;
        assert_eq!(c1.len(), 10);
        assert_eq!(c2.len(), 10);
    }*/
}
